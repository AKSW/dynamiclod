...
import dynlod.LoadedBloomFilters;
...
public class DistributionQueries {
	...
	public ArrayList<DistributionDB> getDistributionsByOutdegree(ArrayList<String> fqdnToSearch,
		...
		try {
			...
			// query all fqdn
			BasicDBObject query = new BasicDBObject(DistributionSubjectDomainsDB.SUBJECT_FQDN,
					new BasicDBObject("$in", fqdnToSearch));

			DBCollection collection = DBSuperClass.getInstance()
					.getCollection(DistributionSubjectDomainsDB.COLLECTION_NAME);
			...
			while (cursor.hasNext()) {
				...
				DistributionDB distribution = new DistributionDB(
						((Number) instance.get(DistributionSubjectDomainsDB.DISTRIBUTION_ID)).intValue());
				list.add(distribution);

				if (!fqdnPerDistribution.containsKey(distribution.getDynLodID())) {
					fqdnPerDistribution.put(distribution.getDynLodID(),
							createDistributionFQDNObject(distribution.getDynLodID()));
	...
	public DistributionFQDN createDistributionFQDNObject(int distribution) {
		...
		BasicDBObject subjectQuery = new BasicDBObject(DistributionSubjectDomainsDB.DISTRIBUTION_ID, distribution);
		...
		while (cursor.hasNext()) {
			subjectsFQDN.add(cursor.next().get(DistributionSubjectDomainsDB.SUBJECT_FQDN).toString());
		}

		// doing the same for objects fqdn
		BasicDBObject objectQuery = new BasicDBObject(DistributionObjectDomainsDB.DISTRIBUTION_ID, distribution);

		collection = DBSuperClass.getInstance().getCollection(DistributionObjectDomainsDB.COLLECTION_NAME);
		...
		while (cursor.hasNext()) {
			objectsFQDN.add(cursor.next().get(DistributionObjectDomainsDB.OBJECT_FQDN).toString());
		}
	...
	public ArrayList<DistributionDB> getDistributionsByIndegree(ArrayList<String> fqdnToSearch,
		...
		try {
			...
			BasicDBObject query = new BasicDBObject(DistributionObjectDomainsDB.OBJECT_FQDN,
					new BasicDBObject("$in", fqdnToSearch));

			DBCollection collection = DBSuperClass.getInstance()
					.getCollection(DistributionObjectDomainsDB.COLLECTION_NAME);
			...
			while (cursor.hasNext()) {
				DBObject instance = cursor.next();
				DistributionDB distribution = new DistributionDB(
						((Number) instance.get(DistributionObjectDomainsDB.DISTRIBUTION_ID)).intValue());
...
//				query3 = new BasicDBObject(DistributionDB.IS_VOCABULARY, isVocabulary);
	...
	public ArrayList<DistributionDB> getDistributionsByTopDatasetID(String topDataset) {
		...
		while (inst.hasNext()) {
			datasetList.add(((Number) inst.next().get(DatasetDB.DYN_LOD_ID)).intValue());
		}
	...
	public ArrayList<DistributionDB> getSetOfDistributions(HashSet<Integer> set) {
		...
		try {
			collection = DBSuperClass.getInstance().getCollection(DistributionDB.COLLECTION_NAME);
			DBCursor instances = collection
					.find(new BasicDBObject(DistributionDB.DYN_LOD_ID, new BasicDBObject("$in", set)));
	...
	public HashSet<DistributionDB> getDistributionsByResource(String resource, String type) {
		...
		// get resource fqdn
		String fqdn = FQDN.getFQDNFromString(resource);

		if (type.equals(DynlodGeneralProperties.TYPE_SUBJECT)) {
			DBCollection collection = DBSuperClass.getInstance()
					.getCollection(DistributionSubjectDomainsDB.COLLECTION_NAME);

			DBObject query;
			query = new BasicDBObject(DistributionSubjectDomainsDB.SUBJECT_FQDN, fqdn);

			DBCursor instances = collection.find(query);

			for (DBObject instance : instances) {
				DistributionDB d = new DistributionDB(
						Integer.valueOf(instance.get(DistributionSubjectDomainsDB.DISTRIBUTION_ID).toString()));
				
				if(LoadedBloomFilters.querySubject(d, resource))
					setOfDistributionFQDN.add(d);
			}
		}
		
		else if (type.equals(DynlodGeneralProperties.TYPE_OBJECT)) {
			DBCollection collection = DBSuperClass.getInstance()
					.getCollection(DistributionObjectDomainsDB.COLLECTION_NAME);

			DBObject query;
			query = new BasicDBObject(DistributionObjectDomainsDB.OBJECT_FQDN, fqdn);

			DBCursor instances = collection.find(query);
			
			for (DBObject instance : instances) {
				DistributionDB d = new DistributionDB(
						Integer.valueOf(instance.get(DistributionObjectDomainsDB.DISTRIBUTION_ID).toString()));
				
				if(LoadedBloomFilters.queryObject(d, resource))
					setOfDistributionFQDN.add(d);
			}
		}
		
		else if (type.equals(DynlodGeneralProperties.TYPE_PROPERTY)) {
			DBCollection collection = DBSuperClass.getInstance()
			...
			if(instances.iterator().hasNext())
				predicateID = Integer.parseInt(instances.iterator().next().get(new AllPredicatesDB().DYN_LOD_ID).toString()); 
			else 
