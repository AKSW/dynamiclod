...
import dynlod.mongodb.objects.EvaluationMongoDBObject;
...
//				if(linksetMongoDBObject.getLinks()>0 && linksetMongoDBObject.getOntologyLinks() ==0){
					try {
						if(t.tm==null)
							t.load(getFile(DynlodGeneralProperties.BASE_PATH+"tree/"+FileUtils.stringToHash(linksetMongoDBObject.getSubjectsDistributionTarget())));
						t.SearchElements(DynlodGeneralProperties.OBJECT_FILE_DISTRIBUTION_PATH+getFile(linksetMongoDBObject.getObjectsDistributionTarget()));
...
//				}
	...
	public void makeTrees() {
		...
		loadLinksets();
		String treePath = DynlodGeneralProperties.BASE_PATH + "/tree/";
...
//			string = FileUtils.stringToHash(string);
			...
			try {
				...
				if (!f.exists()) {
					treeMap.AddElements(DynlodGeneralProperties.SUBJECT_FILE_DISTRIBUTION_PATH
							+ string);
...
//					+ objectFileName;
			objectFileName = DynlodGeneralProperties.BASE_PATH+"objects/"
					+ objectFileName;
...
//						+ subjectFileName;
				subjectFileName = DynlodGeneralProperties.BASE_PATH+"subjects/"
						+ subjectFileName;
